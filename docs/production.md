...
attestation, in one command. Its source is
[`www/install.sh`](https://github.com/blox-eng/openblox/blob/main/www/install.sh),
which is the file `https://openblox.sh/install.sh` serves; read it before you pipe
it anywhere, here most of all:

```sh
OPENBLOX_VERSION=v0.8.1 sh -c "$(curl -fsSL https://openblox.sh/install.sh)"
openbloxd --version      # must print $OPENBLOX_VERSION, not "dev"
```
...
The same steps by hand, which is what to run when you want to see each one
succeed separately, or when `gh` is unavailable and the attestation has to be
checked another way:
